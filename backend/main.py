...
def get_all_urls(base_url: str) -> List[str]:
    ...
    while to_visit:
        ...
        try:
            ...
            if len(urls) > 100:  # reasonable limit for most books
                print(f"Reached URL limit of 100, stopping crawl")
...
def extract_text_from_url(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Limit text size to prevent memory errors (100KB limit)
        if len(text_content) > 100000:
            print(f"Warning: Content from {url} too large ({len(text_content)} chars), truncating to 100KB")
...
def main(base_url: str = None, query: str = None):
    """
    Main function to orchestrate the RAG indexing process or perform search
    """
    if query:
        ...
        for i, result in enumerate(results, 1):
            print(f"{i}. [{result['title']}]({result['url']}) - Score: {result['similarity_score']:.3f}")
            print(f"   Preview: {result['text'][:200]}...")
            print()
        return results
    elif base_url:
        # Validate URL input
        if not isinstance(base_url, str) or not is_valid_url(base_url):
            logger.error(f"Invalid URL provided: {base_url}")
            print(f"Error: Invalid URL provided: {base_url}")
            return None
        base_url = base_url.strip()

        # Perform indexing if base_url is provided
        print(f"Starting RAG indexing for: {base_url}")
        logger.info(f"Starting RAG indexing for: {base_url}")

        # Step 1: Get all URLs from the Docusaurus site
        print("Step 1: Fetching all URLs...")
        logger.info("Step 1: Fetching all URLs...")
        ...
        print("Step 3: Processing URLs and generating embeddings...")
        logger.info("Step 3: Processing URLs and generating embeddings...")
        chunk_id = 0
        ...
        for i, url in enumerate(urls):
            ...
            if not content["text"].strip():
                ...
                logger.info(f"  No content found for {url}, skipping...")
                continue

            print(f"  Content size: {len(content['text'])} characters")
            logger.info(f"  Content size: {len(content['text'])} characters for {url}")
            ...
            try:
                ...
                print(f"  Memory error during chunking, skipping this URL")
                logger.error(f"  Memory error during chunking for {url}")
                continue
            ...
            try:
                ...
                print(f"  Memory error during embedding, skipping this URL")
                logger.error(f"  Memory error during embedding for {url}")
                continue
        ...
        print(f"Indexing completed! Stored {chunk_id} chunks in Qdrant collection '{collection_name}'")
        logger.info(f"Indexing completed! Stored {chunk_id} chunks in Qdrant collection '{collection_name}'")
    else:
        print("Please provide either a base_url for indexing or a query for search")
        return None


if __name__ == "__main__":
    import sys
    # Check if we're running in search mode or indexing mode
    if len(sys.argv) > 1 and sys.argv[1] == "search":
        # Search mode: python main.py search "your query here"
        if len(sys.argv) > 2:
            query = " ".join(sys.argv[2:])
            main(query=query)
        else:
            print("Please provide a search query: python main.py search 'your query'")
    else:
        # Indexing mode: python main.py (uses default URL) or python main.py "URL"
        if len(sys.argv) > 1:
            base_url = sys.argv[1]
        else:
            # Use the provided URL from the user's request as default
            base_url = "https://hackathon-physical-ai-humanoid-robo-ten.vercel.app/docs/intro"
